...
(() => {
  ...
  const state = {
    ...
    lessonDiceImageCache: new Map(),  // prompt -> data URL
  };
  ...
  // saveCurrentLessonDraft({ markDirty: true }) 로 호출하면 "사용자가 직접 편집한 draft" 표시.
  // dirty 표시가 있어야 lesson 2 가 자기 draft 를 lesson 1 인계보다 우선시함.
  function saveCurrentLessonDraft(opts = {}) {
    if (!state.currentLesson) return;
    const editor = $('#editor-textarea');
    if (!editor) return;
    const value = editor.value;
    if (typeof value === 'string' && value.trim()) {
      const lessonKey = Number(state.currentLesson);
      state.lessonDrafts.set(lessonKey, value);
      if (opts.markDirty) {
        state.lessonDrafts.set('__dirty:' + lessonKey, true);
      }
    }
  }
  ...
  function getComposedLessonDoc(lessonNo, baseDoc) {
    ...
    // lesson 2 만 lesson 1 draft 인계 — lesson 3 은 worldmap·40칸 고정이라 인계 X.
    const allowsInherit = Number(lessonNo) === 2;
    const prevDraft = allowsInherit ? state.lessonDrafts.get(1) : null;

    // 우선순위:
    //  1) 자기 차시 draft 가 있고, 그 안에 user-dirty 표시가 있으면 → draft 사용
    //  2) lesson 1 draft 가 있으면 → mergeInheritedLessonDoc(baseDoc, lesson1) 으로 재인계
    //  3) 아무것도 없으면 → baseDoc
    // (draft 만 있고 dirty 표시가 없으면 = 단순 방문 후 lesson 1 가 갱신된 상황 → 재인계)
    if (draft && state.lessonDrafts.get('__dirty:' + Number(lessonNo))) {
      console.info(`[lesson 인계] ${lessonNo}차시 — 사용자 편집 draft 사용`);
      return draft;
    }
    if (prevDraft) {
      console.info(`[lesson 인계] ${lessonNo}차시 — lesson 1 draft 인계 적용`);
  ...
  async function selectLesson(lessonNo) {
    ...
    // 차시 전환 직전 — 학생이 OLD lesson 에 편집해뒀을 가능성 높음 → markDirty
    saveCurrentLessonDraft({ markDirty: true });
    ...
    try {
      ...
      if (typeof refreshFeatureGateButtons === 'function') refreshFeatureGateButtons();
      // 차시별 도구바 비활성화 (1차시: 음악, 2차시: 캐릭터, 3차시: 배경) — refreshCharacterButtonState 이후 호출해 덮어씀
      refreshLessonToolbarGates();
      editor.focus();
  ...
  const MARBLE_TEMPLATE_PATH = './assets/samples/bluemarble/index.html';
  const MARBLE_TEMPLATE_BASE = `${window.location.origin}/assets/samples/bluemarble/`;
  const MARBLE_IP_META = {
    '데니스': { id: 'dennis', emoji: '👦', imageUrl: '../../characters/archive/dennis.png' },
    '슬기': { id: 'seulgi', emoji: '👧', imageUrl: '../../characters/archive/seulgi.png' },
  };
  ...
  // resolveNamedColor 와 동일하지만 매칭 실패 시 null 반환 (fallback X).
  // 호출자가 "로컬 매칭 성공 여부" 를 알아야 할 때 사용 — pin 색상 merge 시 Claude 결과 반영 판단.
  function resolveNamedColorOrNull(text) {
    const normalized = String(text || '').trim().toLowerCase();
    if (!normalized) return null;
    if (/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(normalized)) return normalized;
    return SIMPLE_COLOR_MAP[normalized] || SIMPLE_COLOR_MAP[normalized.replace(/\s+/g, '')] || null;
  }
  ...
  function parseLessonOnePins(text) {
    ...
    const fallback = [
      { name: '나', color: '#E63946', isHuman: true, id: 'player-human', colorMatched: true },
      { name: '친구', color: '#3D7BA3', isHuman: false, id: 'player-ai', colorMatched: true },
    ];
    ...
    const parsed = lines.map((line, index) => {
      ...
      return {
        name: name || base.name,
        color: matchedColor || base.color,
        // 매칭 실패면 false → merge 단계에서 Claude 결과로 덮어쓸 수 있게 표시
  ...
  function parsePlayers(text, lessonNo) {
    if (Number(lessonNo) === 1 || Number(lessonNo) === 2) {
      return parseLessonOnePins(text).map((player, index) => ({
        ...
        // 로컬 매칭 성공 여부 — false 면 merge 단계에서 Claude 결과로 덮어씀
  ...
  function parseCities(text) {
    ...
    return block
      ...
      .filter((line) => /^-\s+/.test(line))
      .map((line) => {
        // 1) 인덱스 + 도시: 랜드마크 형식 (예: "- 7칸 : 영국 런던: 빅벤 / 6200골드")
        ...
        // 2) 인덱스 + 단일 라벨 형식 (예: "- 7칸 : 유나네 집 / 5000골드")
        //    학생이 "도시: 랜드마크" 콜론 형식을 안 지켜도 칸 번호로 매칭되도록.
        const indexedSimpleMatch = line.match(/^-\s*(\d+)\s*칸\s*:\s*(.+?)\s*\/\s*([\d,]+)골드/);
        if (indexedSimpleMatch) {
          return {
            index: Number(indexedSimpleMatch[1]),
            label: indexedSimpleMatch[2].trim(),
            landmark: indexedSimpleMatch[2].trim(),
            toll: numberFromText(indexedSimpleMatch[3], 5000),
          };
        }
        // 3) 인덱스 없이 도시: 랜드마크 (freeSource — 도시 칸 순서대로 배치)
  ...
  // "### 음악" 섹션의 첫 비-기본 라인을 AI 작곡 프롬프트로 추출.
  // "기본 음악" 또는 빈 문자열은 무시.
  function parseLessonMusicPrompt(text) {
    const block = sectionBlock(text, '음악') || sectionBlock(text, '배경음악');
    if (!block) return '';
    const lines = block.split('\n').map((l) => l.trim()).filter(Boolean);
    for (const raw of lines) {
      const value = raw.replace(/^-\s*/, '').trim();
      if (!value || /^기본\s*음악$/.test(value)) continue;
  ...
  function parseMarbleLessonConfig(text, lessonNo) {
    ...
    const lessonOneDice = (isPinLesson || Number(lessonNo) === 3) ? parseLessonOneDice(text) : null;
    return {
      title: title.replace(/^\d+차시\s*[^\s]+\s*/, '').trim(),
      board: {
        backgroundImageUrl: parseBoardImage(text) || (Number(lessonNo) === 3 ? `${window.location.origin}/assets/backgrounds/worldmap.jpg` : ''),
      ...
      ui: {
        ...
        // 3차시는 출발 칸(idx 0)을 6시 방향(보드 하단)으로 — 보드 전체 180° 회전
        boardRotation: Number(lessonNo) === 3 ? 180 : 0,
        // 3차시는 도시 매입 시 학생(human) 에게 [구입]/[지나가기] 선택 모달 표시
        cityBuyChoice: Number(lessonNo) === 3,
      },
    };
  }
  ...
  // "### 주사위" 본문이 비-기본 일 때 AI 이미지 프롬프트로 추출.
  function parseDicePrompt(text) {
    const block = sectionBlock(text, '주사위');
    if (!block) return '';
    const line = block.split('\n').map((v) => v.trim()).find(Boolean) || '';
    const value = line.replace(/^-\s*/, '').trim();
    if (!value || /^기본\s*주사위$/.test(value)) return '';
    return value.slice(0, 200);
  }

  async function resolveLessonOneDiceImage(sourceText) {
    const prompt = parseDicePrompt(sourceText);
    if (!prompt) return null;
    if (state.lessonDiceImageCache.has(prompt)) {
      console.info('[주사위 이미지] 캐시 히트:', prompt);
      return state.lessonDiceImageCache.get(prompt);
    }
    console.info('[주사위 이미지] 생성 시작:', prompt);
    const t0 = Date.now();
    try {
      const res = await fetch('/api/lesson-background', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt, kind: 'dice' }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data?.imageUrl) {
        console.warn('[주사위 이미지] 실패:', res.status, data?.error, data?.message);
        return null;
      }
      console.info(`[주사위 이미지] 생성 성공 (${Date.now()-t0}ms, ${(data.imageUrl.length/1024).toFixed(0)}KB)`);
      state.lessonDiceImageCache.set(prompt, data.imageUrl);
      return data.imageUrl;
    } catch (err) {
      console.warn('[주사위 이미지] 네트워크 예외:', err);
      return null;
    }
  }
  ...
  async function buildPatchedMarbleHtml(sourceText, lessonNo, options = {}) {
    ...
    const isPinLesson = Number(lessonNo) === 1 || Number(lessonNo) === 2;
    const allowsDice = isPinLesson || Number(lessonNo) === 3;

    // AI 호출 5종을 병렬로 발사 — 가장 느린 호출 시간만큼만 대기 (보통 이미지 ~10s)
    const [diceFromClaude, pinsFromClaude, boardColorFromClaude, backgroundImageUrl, diceImageUrl] = await Promise.all([
      allowsDice && options.useAiDice ? resolveLessonOneDiceWithClaude(sourceText) : null,
      isPinLesson && options.useAiPins ? resolveLessonOnePinColorsWithClaude(sourceText) : null,
      isPinLesson && options.useAiBoardColor ? resolveLessonOneBoardColorWithClaude(sourceText) : null,
      isPinLesson && options.useAiBackgroundImage ? resolveLessonOneBackgroundImage(sourceText) : null,
      allowsDice && options.useAiDice ? resolveLessonOneDiceImage(sourceText) : null,
    ]);

    if (diceFromClaude) {
      // 로컬 파서가 키워드(별/하트/번개) 매칭에 성공한 경우만 explicit 로 간주.
      // 그 외(label 만 다른 경우 — 예: "강아지 주사위") 는 Claude 가 보정해야 함.
      const localThemeRecognized = config.dice.theme !== 'classic';
      if (!localThemeRecognized) {
        config.dice = { ...config.dice, ...diceFromClaude };
      }
    }
    if (pinsFromClaude) {
      // 로컬 파서가 색상 키워드 매칭에 성공한 경우만 local 우선.
      // 그 외 (예: "핑크" 가 SIMPLE_COLOR_MAP 에 없어서 fallback 으로 떨어진 경우) Claude 결과로 덮어씀.
      config.players = config.players.map((player, index) => {
        const claudeColor = pinsFromClaude[index]?.color;
        const finalColor = player.pinColorMatched
          ? player.pinColor
          : (claudeColor || player.pinColor);
        return { ...player, pinColor: finalColor };
      });
    }
    if (boardColorFromClaude && typeof boardColorFromClaude === 'object') {
    ...
    if (diceImageUrl) config.dice.imageUrl = diceImageUrl;

    const injected = `<base href="${MARBLE_TEMPLATE_BASE}"><script>window.__GONGDO_MARBLE_CONFIG__=${JSON.stringify(config)};</script>`;
    let html = template.replace(/<head>/i, `<head>${injected}`);
    if (options.musicScore) html = injectBgmIntoMarbleHtml(html, options.musicScore);
    return html;
  }

  // BGM 주입 — chat.js 의 injectBgmIntoGame 과 동일 패턴.
  // 인앱 iframe 에서는 sandbox 자동재생 차단으로 부모(GongdoBGM) 가 재생을 담당하므로
...
<\/script>`;
  ...
  async function handleStartClick() {
    ...
    if (state.currentLesson >= 1 && state.currentLesson <= 4) {
      showGeneratingModal();
      try {
        // [▶ 시작] 누르는 시점 = 학생이 편집 완료한 의도 → markDirty
        saveCurrentLessonDraft({ markDirty: true });
        const isPinLesson = Number(state.currentLesson) === 1 || Number(state.currentLesson) === 2;
        const useAiDice = (isPinLesson || Number(state.currentLesson) === 3) && isLessonDocumentModified();
        const useAiPins = isPinLesson && isLessonDocumentModified();
        // lesson 1·2 모두 "보드 색상" 변경을 무조건 이미지 생성으로 보내므로 색상 폴백을 끔.
        // (Claude board_color 가 색상을 채우면 이미지 실패 시 의도와 다른 색이 보임)
        ...
        const [html, generatedScore] = await Promise.all([
          buildPatchedMarbleHtml(editor.value, state.currentLesson, { useAiDice, useAiPins, useAiBoardColor, useAiBackgroundImage, musicScore }),
          docMusicPrompt ? window.GongdoBGM.generateAndApply(docMusicPrompt) : Promise.resolve(null),
          wait(5000),
        ]);
        ...
        // [시작] 클릭 제스처로 AudioContext prewarm (재생은 iframe 로드 후 부모가 담당)
        if (musicScore) {
          try { await window.GongdoBGM.prewarmAudio(); } catch {}
        }

        state.lastGeneratedHtml = finalHtml;
        state.lastGeneratedHtmlSnapshot = { html: finalHtml, sourceText: editor.value };
        state.promptHistory.push({
          at: new Date().toISOString(),
          lessonNo: state.currentLesson,
          document: editor.value.slice(0, 3000),
        });
        launchGame(finalHtml);
        // 부모 페이지에서 BGM 재생 (iframe 안 player 는 sandbox 에서 자동재생 막힘 — 부모가 담당)
        if (musicScore) {
...
<style data-injected="gongdo-viewport-lock">
  ...
  function initTutor() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // ⚠️ findEditorLineByHint 는 부수효과 없음 (line number 만 계산).
        // 옛 코드는 highlightEditorByHint 가 자동 실행되어 editor.focus() 가 학생 타자를
        // 가로채는 버그가 있었음 (튜터 input 에 타자한다고 생각하는 동안 실제로는 editor 에
        // 입력되어 도시 목록이 사라지는 현상). 자동 focus/selection 제거 — 학생이 배지를
        // 클릭할 때만 highlightEditorByHint 가 실제로 focus·selection 적용.
        const { displayText, hint } = parseTutorHint(data.reply || '...');
        const lineNumber = hint ? findEditorLineByHint(hint) : null;
  ...
  function appendTutorMessage(text, sender, opts = {}) {
    ...
    if (sender === 'bot') {
      ...
      img.src = './assets/characters/archive/dennis.png';
      img.alt = '';
  ...
  const IP_ASSET_BASE = '/assets/characters';
  const IP_META = {
  ...
  // 차시별 도구바 비활성화 — 1차시: 음악, 2차시: 캐릭터, 3차시: 배경.
  // 캐릭터는 refreshCharacterButtonState 가 단독으로 차시별 lock 상태를 관리하니
  // 이 함수에서는 .is-locked 만 추가/유지하고 cleanup 은 하지 않는다.
  // bgm·theme 는 refreshFeatureGateButtons 가 unlock 버튼 부재로 skip 하므로 우리가 단독 관리 → 매번 reset 후 재적용.
  function refreshLessonToolbarGates() {
    const lessonNo = Number(state.currentLesson);

    // bgm·theme 는 우리가 단독 관리하므로 매 호출마다 잠금 상태 초기화
    ['#btn-bgm', '#btn-theme'].forEach((sel) => {
      const btn = $(sel);
      if (!btn) return;
      btn.disabled = false;
      btn.classList.remove('is-locked');
      btn.removeAttribute('title');
      btn.setAttribute('aria-label', sel === '#btn-bgm' ? '배경음악 열기' : '배경 고르기');
      const icon = btn.querySelector('.character-lock-icon');
      if (icon) icon.hidden = true;
    });

    const gates = {
      1: { sel: '#btn-bgm',       title: '음악 (이 차시에서는 사용 불가)' },
      2: { sel: '#btn-character', title: '캐릭터 (이 차시에서는 사용 불가)' },
      3: { sel: '#btn-theme',     title: '배경 (이 차시에서는 worldmap 고정)' },
    };
    const target = gates[lessonNo];
    if (!target) return;
    const targetBtn = $(target.sel);
    if (!targetBtn) return;

    targetBtn.disabled = true;
    targetBtn.classList.add('is-locked');
    targetBtn.setAttribute('title', target.title);
    targetBtn.setAttribute('aria-label', target.title);
    const lockIcon = targetBtn.querySelector('.character-lock-icon');
    if (lockIcon) lockIcon.hidden = false;

    // 열려있던 popover 가 있으면 닫기 (aria-expanded 동기화)
    const popoverId = targetBtn.getAttribute('aria-controls');
    if (popoverId) {
  ...
  function ensureEditorMirror(editor) {
    ...
    editor.addEventListener('input', () => {
      ...
      // 자동 저장 — 타자할 때마다 lessonDrafts 에 보존 + dirty 표시.
      // dirty 표시가 있어야 lesson 2 가 자기 draft 를 lesson 1 인계보다 우선시함.
      // (튜터/포커스 사고로 문서가 망가져도 차시 전환 시 마지막 입력 복구 가능)
      saveCurrentLessonDraft({ markDirty: true });
    });
    return mirror;
  }
  ...
  //   ① 음악 적용 + 재생 중 → 정지
  //   ② 음악 적용 + 정지 중 → 재생
  //   ③ 음악 미적용 → popover 열기 (기본 음악 / AI 생성 흐름)
  function initBgmPanel() {
    const btn = $('#btn-bgm');
    const popover = $('#bgm-popover');
    const closeBtn = $('#bgm-popover-close');
    ...
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const applied = window.GongdoBGM?.getAppliedScore?.();
      if (applied) {
        const isPlaying = !!window.GongdoBGM?.state?.isPlaying;
        if (isPlaying) {
          try { window.GongdoBGM.stop(); } catch {}
        } else {
          try { await window.GongdoBGM.prewarmAudio?.(); } catch {}
          try { await window.GongdoBGM.play(applied, 'applied'); } catch {}
        }
        return;
      }
      // 음악 미적용 → popover 토글
  ...
  function insertThemeIntoDoc(label, desc, colors) {
    ...
    let insertStart;
    if (sectionMatch) {
      const headingLine = sectionMatch[1];
      const sectionStart = sectionMatch.index;
      const bodyEnd = sectionMatch.index + sectionMatch[0].length;
      insertStart = sectionStart + headingLine.length;
      nextValue = current.slice(0, sectionStart) + headingLine + newLine + '\n' + current.slice(bodyEnd);
    } else {
      // 호환 fallback (lesson 3 등): 기존 "- 배경:" 줄 교체 또는 끝에 추가
      const legacyLine = `- 배경: ${label} (${desc}) · 색상: ${colors}`;
      const legacyRegex = /^-?\s*배경:.*$/m;
      const legacyMatch = current.match(legacyRegex);
      if (legacyMatch) {
        insertStart = legacyMatch.index;
        nextValue = current.slice(0, legacyMatch.index) + legacyLine + current.slice(legacyMatch.index + legacyMatch[0].length);
      } else {
        const trailing = current.endsWith('\n') ? '' : '\n';
        insertStart = current.length + trailing.length;
        nextValue = current + trailing + legacyLine + '\n';
      }
    }
    editor.value = nextValue;

    const inserted = sectionMatch ? newLine : `- 배경: ${label} (${desc}) · 색상: ${colors}`;
    const insertEnd = insertStart + inserted.length;
    editor.focus();
  ...
  function insertCharacterIntoDoc(label, file) {
    ...
    const newLine = `- 주인공: ${label} (이미지: ${origin}/assets/characters/${file})`;
    const current = editor.value;
    const regex = /^-?\s*주인공:.*$/m;
  ...
  // ─────────── 테스트용 내부 익스포트 (production 영향 0) ───────────
  // tests/_loadApp.mjs 가 globalThis.__GONGDO_TEST_EXPORTS = {} 를 미리 설정한 환경에서만 실행.
  // 일반 브라우저/배포에선 globalThis.__GONGDO_TEST_EXPORTS 가 undefined 라 if 블록 자체가 no-op.
  if (typeof globalThis !== 'undefined' && globalThis.__GONGDO_TEST_EXPORTS) {
    Object.assign(globalThis.__GONGDO_TEST_EXPORTS, {
      // utility
      sectionBlock, extractSectionBlock, replaceSectionBlock, mergeInheritedLessonDoc,
      numberFromText, resolveNamedColor, resolveNamedColorOrNull,
      // parsers — pure (input string → output object)
      parseLessonOnePins, parseLessonOneDice, parseDicePrompt, parseLessonMusicPrompt,
      parseLessonBackgroundPrompt, parseLessonOneBoardColor, parseLessonOneBackgroundColor,
