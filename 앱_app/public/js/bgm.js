...
(() => {
  ...
  function updateButtonState() {
    ...
    if (toolbarBtn) {
      ...
      if (toolbarLabel) {
        // 음악 적용 + 재생/정지 상태에 따라 라벨 변경 (정지 ↔ 실행 토글)
        if (state.appliedToGame) {
          toolbarLabel.textContent = state.isPlaying ? '음악 정지' : '음악 실행';
        } else {
          toolbarLabel.textContent = '음악';
        }
      }
      // 음악 아이콘도 상태에 따라 — 재생 중이면 ⏸, 정지 중이면 ▶
      const iconSpan = toolbarBtn.querySelector('span[aria-hidden="true"]:not(.character-lock-icon)');
      if (iconSpan) {
        if (state.appliedToGame) {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          if (data.rateLimited) {
            ...
            // 자동 적용 — 생성과 동시에 게임에 들어감 (수동 [✅ 넣기] 단계 제거).
            state.appliedToGame = { score: data.score, mood: state.lastGeneratedMood };
            await play(data.score, 'ai');
            statusEl.className = 'bgm-ai-status is-success';
            statusEl.textContent = `✅ '${state.lastGeneratedMood}' 음악이 자동으로 게임에 들어갔어요! 도구바 [⏸/▶] 버튼으로 정지/실행할 수 있어요 🎶`;
            updateAppliedBadge();
            updateButtonState();
            // [▶ 시작] 버튼 attention — 학생 시선 유도
            try { window.GongdoApp?.highlightStartButton?.('bgm'); } catch {}
          }
        } catch (err) {
          statusEl.className = 'bgm-ai-status is-error';
  ...
  window.GongdoBGM = {
    ...
    // lesson 문서의 "### 음악" 프롬프트로 자동 작곡 + appliedToGame 설정.
    // popover 흐름과 별개로 [▶ 시작]에서 호출됨.
    async generateAndApply(prompt) {
      if (!prompt || typeof prompt !== 'string') return null;
      const data = await generateFromPrompt(prompt.trim()).catch(() => null);
      if (!data || data.error || data.rateLimited || !data.score) return null;
      state.appliedToGame = { score: data.score, mood: prompt.slice(0, 60) };
      return data.score;
    },
  };
})();
